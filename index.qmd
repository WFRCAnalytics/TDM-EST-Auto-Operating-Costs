...
The analysis uses data from multiple federal sources to calculate vehicle operating costs. The Bureau of Transportation Statistics (BTS) provides fuel prices, vehicle expenditures, and fleet characteristics. The Energy Information Administration (EIA) supplies regional fuel price data for the Rocky Mountain area, which includes Utah. The Bureau of Labor Statistics (BLS) provides Consumer Price Index data to adjust for inflation. Together, these datasets enable a comprehensive view of how auto operating costs have evolved and allow us to project costs for the base year.

Data collection is automated where possible. Each section downloads the latest data from authoritative sources and caches it locally to avoid repeated downloads. The code checks if files already exist before fetching new data, making the analysis reproducible while minimizing unnecessary network requests.
...
Raw data must be processed and combined to produce meaningful cost estimates. This section derives key metrics by joining datasets, calculating trends, and comparing actual values against historical patterns. Each subsection focuses on a specific cost component: transportation expenditures as a share of household budgets, the split between variable and fixed auto costs, fuel economy trends across vehicle classes, and regional fuel prices.

### Transportation Expenditures

Transportation expenditures provide context for understanding how much households allocate to vehicle-related expenses. By examining both total transportation costs and the gasoline component separately, we can assess whether our operating cost estimates align with national spending patterns.
...
Vehicle operating costs divide naturally into variable costs (fuel, maintenance) that change with miles driven and fixed costs (insurance, depreciation, registration) that remain constant regardless of usage. The American Automobile Association tracks these costs annually for a vehicle driven 15,000 miles per year, providing a benchmark for our calculations.
...
This section isolates the two primary variable cost components: gasoline and maintenance plus tires. These costs directly relate to vehicle usage and vary with fuel prices and vehicle condition. Trend analysis reveals whether costs are rising, falling, or remaining stable over time.
...
Average annual vehicle miles traveled varies significantly by truck class. Light trucks typically accumulate fewer miles than heavy combination trucks used for long-haul freight. Understanding these patterns helps calibrate operating costs to realistic usage scenarios for different vehicle types.
...
Fuel economy directly affects operating costs—vehicles that travel more miles per gallon cost less to operate. Federal data on fleet-wide fuel economy shows how efficiency has improved over time due to technological advances and regulatory standards. We track trends separately for light-duty vehicles, medium-duty trucks, and heavy-duty trucks.
...
Regional fuel prices fluctuate based on crude oil markets, refining capacity, distribution costs, and seasonal demand. The Energy Information Administration publishes weekly retail prices for the Rocky Mountain region, which provides the most relevant pricing for our study area. Both gasoline and diesel prices must be considered since different vehicle classes use different fuels.
...
The final calculation combines fuel costs (price per gallon divided by miles per gallon) with maintenance costs to produce operating costs in cents per mile. We calculate separate rates for automobiles, light trucks, medium-duty trucks, and heavy-duty trucks, reflecting their different fuel types, efficiency levels, and maintenance requirements.
...
The resulting operating costs represent the variable cost per mile for each vehicle class in the base year. These values can be compared to previous years to assess cost trends and inform travel demand model assumptions about how travelers respond to changes in auto operating expenses.
