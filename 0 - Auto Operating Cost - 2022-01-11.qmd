...
df_TransportationExpenditure = pd.DataFrame({
    ...
    "Transportation Percent of Total": df_PersonalExpenditure.loc[2, str(year_range[0]):str(year_range[-1])].values,
    # Extract the 'Gasoline Cost of Transportation' for the years 1990-2020 from row 2
    "Gasoline Cost of Transportation": df_AverageCost.loc[2, str(year_range[0]):str(year_range[-1])].values
})
...
# FIXME: the trend lines are not working
...
df_AutoCost_VariableFixed = pd.DataFrame({
    "Year": year_range,
    "Variable": df_AverageCost.loc[6, str(year_range[0]):str(year_range[-1])].values,
    "Fixed": df_AverageCost.loc[7, str(year_range[0]):str(year_range[-1])].values,
    "Total": df_AverageCost.loc[5, str(year_range[0]):str(year_range[-1])].values,
    "CPI": df_CPI[df_CPI['Year'].between(year_range[0], year_range[-1])]["Avg."].values
})
